...
# tracking (moves towards the mc)
# if the mc is hit loeses a life
...
def placePoint(app):
    ...
    while(len(app.points) < 3):
        ...
        if ((row,col) != (app.chara.row, app.chara.col) and 
                         (row < app.rows - 2) and
                         ((row, col) not in app.obstaclesLocation)
                         and (row, col) not in app.points):
...
def moveChara(app, event):
    ...
    if (event.key == 'Up' and (0 <= app.chara.row - 1 < app.rows - 2) 
        and (app.chara.row - 1, app.chara.col) not in app.obstaclesLocation):    
        app.chara.row -= 1
    elif (event.key == 'Down' and 0 <= app.chara.row + 1 < app.rows - 2
          and (app.chara.row + 1, app.chara.col) not in app.obstaclesLocation):  
        app.chara.row += 1
    elif (event.key == 'Left' and 0 <= app.chara.col - 1 < app.cols
          and (app.chara.row, app.chara.col - 1) not in app.obstaclesLocation):  
        app.chara.col -= 1
