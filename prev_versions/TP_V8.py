...
def appStarted(app):
    ...
    app.gridHeight = app.height - app.textboxHeight - app.margin
...
class mc(object):
    def __init__(self, app):
        ...
        # fix this laterrrr
        self.y = app.gridHeight/2 + app.mcSize/2
...
class carnegie(object):
    def __init__(self, app):
        ...
        Ly = [0, app.gridHeight + app.margin]
        self.y = Ly[iy]
...
class tank(object):
    def __init__(self, app):
        # will always generate in the center cell
...
def inExit(app, x, y, size):
    ...
    x0, y0, x1, y1 = app.width - app.margin - 20, app.gridHeight/2 - 20, app.width - app.margin, app.gridHeight/2 + 20
    print(x0, y0, x1, y1)
    print(x, y)
    if (((x + size/2 >= x0 and x + size/2 < x1) or (x - size/2 >= x0 and x - size/2 < x1))
        and ((y + size/2 >= y0 and y + size/2 <= y1) or (y - size/2 >= y0 and y - size/2 <= y1))):
...
def inBounds(app, x, y, size):
    x0, y0, x1, y1 = app.margin, app.margin, app.width - app.margin, app.gridHeight
...
def placePoint(app):
    while(len(app.points) < 3):
        ...
        y = random.randint(app.margin + app.pointSize/2, app.gridHeight)
...
def getCellBounds(app, row, col):
    ...
    gridHeight = app.gridHeight - 2*app.margin
...
def moveChara(app, event):
    ...
    if (event.key == 'Up' and inBounds(app, app.chara.x, app.chara.y - step, app.mcSize)
        ...
        app.chara.row = app.chara.y//((app.gridHeight - 2*app.margin)/app.rows)
        app.chara.y -= step
...
def nextLevel(app):
    if app.p >= app.pointsNeeded and inExit(app, app.chara.x, app.chara.y, app.mcSize):
        ...
        app.chara.y = app.gridHeight/2 + app.mcSize/2
...
def drawTank(app, canvas):
    # he's just going to be a red dot for now
