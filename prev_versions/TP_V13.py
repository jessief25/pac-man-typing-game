...
def appStarted(app):
    ...
    ########################
    # all this sprite stuff heavily based on : https://www.cs.cmu.edu/~112/notes/notes-animations-part4.html#spritesheetsWithCropping
...
def tankIn(app):
    for c in app.tanks:
        if c.row == app.chara.row and c.col == app.chara.col:
            app.chara.lives -= 1
            coordinates = c.x, c.y
            app.tanks.remove(c)
            return coordinates
...
def timerFired(app):
    ...
    if not app.gameOver: 
        ...
        if app.level > 2:
            tankChecks(app)
            coordinates = tankIn(app)
            if coordinates!=None:
                app.sinkTime = 0
                # will make it so that it animates the sink...
                if app.sinkTime < 6 and app.time%5 == 0:
            '''
