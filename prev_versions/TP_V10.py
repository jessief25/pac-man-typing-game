...
'''
features to add:
- level things in general (adjust speed and difficulty ig); introduce certain ghosts at certain levels
...
def initSize(app):
    ...
    app.mcSize = int(app.cellSize/1.25)
    app.carnegieSize = int(app.cellSize/3)
    ...
    if len(app.randos) > 0:
        ...
        for word in app.randWords:
            if "".join(app.text[-1]) == word:
                ...
                app.text.append([""])
                app.p += 1
    #app.randy.move(app, 0)
    #print("position", (app.randy.x, app.randy.y))
...
def nextLevel(app):
    if app.p >= app.pointsNeeded and inExit(app, app.chara.x, app.chara.y, app.mcSize):
        ...
        if app.numObstacles > 0.25*(app.cols * app.rows):
            ...
            app.cols += 1
        if app.level > 2:
            app.carnegieGenerationSpeed -= 10
        if app.level > 5:
            app.tankSpeed -= 2
        if app.level > 6:
            app.randoSpeed -=1
...
def redrawAll(app, canvas):
    ...
    canvas.create_text(5,0, anchor = "nw", text = f'points: {app.p}')
    #prints how many lives mc has
    canvas.create_text(app.width - 10, 0, anchor = "ne", text = f'lives: {app.chara.lives}')
    canvas.create_text(app.width/2, app.margin/2, text = f'LEVEL {app.level}', font = "bold 20")
    pointsNeeded = app.pointsNeeded - app.p
    if pointsNeeded < 0:
        pointsNeeded = 0
    canvas.create_text(app.width/2, app.height - app.textboxHeight - app.margin/2, 
                       text = f"points needed: {pointsNeeded}")
