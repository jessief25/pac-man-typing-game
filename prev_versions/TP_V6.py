...
def initObstacles(app, num):
    ...
    while len(app.obstacles) < num:
        ...
        if (newRow, newCol) not in app.attempted:
            ...
            # finds a solution (not all the solutions...) --> 
            # use backtracking to find all the paths (add to paths set... well would have to be a list)
            # want to also make a set of all the possible places to go and 
            if not isSolution(app, app.chara.row, app.chara.col, set()):
    ...
    if (row, col) == app.exit:
        ...
        for (drow, dcol) in [(-1,0), (1,0), (0,1), (0,-1)]:
            if (0 <= row + drow < (app.rows - 2) and 0 <= col + dcol < app.cols 
                ...
                solution = isSolution1(app, row, col, set())
                if solution != None:
                    #app.possible.add((row, col))
                    return solution
